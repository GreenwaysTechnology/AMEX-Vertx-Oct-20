...
//  }
  ...
  public void start() throws Exception {
    ...
    //another resource ; message resource
    Router messageRouter = Router.router(vertx);
    messageRouter.get("/hello").handler(routingContext -> {
      routingContext.response().end("hello");
    });
    messageRouter.get("/hai").handler(routingContext -> {
      routingContext.response().end("Hai");
    });
    messageRouter.get("/welcome").handler(routingContext -> {
      routingContext.response().end("welcome");
    });

    //Create application Router/Main Router , which should be passed to request
    Router appRouter = Router.router(vertx);
    //binding subrouters with app router
    appRouter.mountSubRouter("/products", productRouter);
    appRouter.mountSubRouter("/message", messageRouter);
